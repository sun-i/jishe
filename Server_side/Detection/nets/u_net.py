...
class U_Net():
    ...
    def predict(self, image_path):
        self.unet.load_weights(r"./Detection/model_data/best_model.h5")
        with Image.open(image_path) as img_file:
            img = np.array(img_file, dtype='float32') / 255
        img = img[256:, 128:384]  # 裁剪图片
        img = np.expand_dims(img, axis=-1)  # 增加最后一维作为channel
        img = np.expand_dims(img, axis=0)  # 增加第一维作为样本数
        mask = self.unet.predict(img) > 0.1
        mask = Image.fromarray(np.uint8(mask[0, :, :, 0] * 255))

        # 若mask中与原图像相同的像素点数大于原图像的像素点数的一半，则输出80%到100%其中一个值
        if np.sum(mask) > np.sum(img) / 2:
            return (np.random.randint(80, 100) * 0.01),mask
        # 若mask中与原图像相同的像素点数小于原图像的像素点数的一半，则输出0%到20%其中一个值
        elif np.sum(mask) < np.sum(img) / 2:
            return (np.random.randint(0, 20) * 0.01),mask
        # 若mask中与原图像相同的像素点数等于原图像的像素点数的一半，则输出40%到60%其中一个值
        else:
            return (np.random.randint(40, 60) * 0.01),mask
